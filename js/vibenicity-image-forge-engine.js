...
window.VibenicityImageForge = {
    ...
    init(apiKey) {
        if (apiKey) {
            ...
            if (!this.config.apiKey && window.VibenicityCodeGenerator && window.VibenicityCodeGenerator.config) {
                this.config.apiKey = window.VibenicityCodeGenerator.config.apiKey || '';
    ...
    async callGemini(model, prompt, files = []) {
        ...
        const parts = [{
            text: `You are Vibenicity V20 Image Forge. Build a production-ready single HTML file.
...
OUTPUT FORMAT: Only output raw HTML code. No markdown, no backticks, no explanations. Just pure HTML starting with <!DOCTYPE html>.`
     ...
     * Export code as ZIP
     */
    async exportAsZip(code, projectName = 'vibenicity-project') {
        if (!window.JSZip) {
...
Generated by Vibenicity Image Forge
...
console.log('✅ VibenicityImageForge loaded');
