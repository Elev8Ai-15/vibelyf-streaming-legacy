 ...
 * Builds single-file HTML apps from a prompt + optional uploaded images.
 * Routes through the VibeLyf Worker (/api/llm/codegen); image uploads are sent
 * as inline_data and served by Claude (multimodal) server-side. No browser key.
 */
...
**Model:** ${this.config.workingModel || 'VibeLyf AI'}
...
`;
    ...
    /**
     * Get engine status. The model chain now lives server-side on the Worker,
     * so we only know which model last served us (from the Worker response).
     */
    getModelStatus() {
        return {
            engine: 'worker',
            working: this.config.workingModel || null
        };
    },
